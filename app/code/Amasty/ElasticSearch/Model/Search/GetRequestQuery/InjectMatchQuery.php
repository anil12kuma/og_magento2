...
class InjectMatchQuery implements InjectSubqueryInterface
{
    ...
    private function getConditionsByMatches(QueryInterface $request, array $requestValue): array
    {
        ...
        foreach ($request->getMatches() as $match) {
            ...
            $conditions[] = $this->getCondition($field, $value, $match, $requestValue);
            ...
            if ($this->config->hasStemming()) {
                ...
                $conditions[] = $this->getCondition($field, $stemmingValue, $match, $requestValue, true);
            }
        }
     ...
     */
    private function getValue(string $value, array $queryConfig, bool $skipWildCardModification = false): string
    {
