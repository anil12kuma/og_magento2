...
abstract class AbstractSocial extends Action
{
    ...
    {
        $this->_logger->debug('testcalling createCustomerProcess ');
    ...
    {
        $this->_logger->debug('testcalling createCustomer ');
    ...
    public function _appendJs($content = null)
    {
        ...
        if ($this->_request->getParam('authen') === 'wishlist') {
            $productid = $this->cookieManager->getCookie('wishlistaddproductlogingoogle');
            $raw = $resultRaw->setContents(
                $content ?: sprintf(
                    "<script>window.opener.socialCallback('%s', window);</script>",
                    $this->_url->getUrl('customwishlist/index/index/').'productid/'.$productid
                )
            );
        }
        elseif ($this->_request->getParam('authen') === 'popup') {
            $raw = $resultRaw->setContents(
                $content ?: sprintf(
                    "<script>window.opener.socialCallback('%s', window);</script>",
        ...
        elseif ($this->_loginPostRedirect()) {
            $raw = $resultRaw->setContents(
                $content ?: sprintf(
                    "<script>window.opener.socialCallback('%s', window);</script>",
