        ...
        $.widget(
            'tigren.ajaxLogin', $.tigren.ajaxSuite, {
                ...
                initEvents: function () {
                    ...
                    $('body').on(
                        'click', self.options.ajaxLogin.loginElmSelector, function (e) {
                            e.preventDefault();
                            customredirectUrl = $(this).attr('data-redirect');
                            if(customredirectUrl == 'checkout'){
                                $('.account-social-login').find('.login-social').hide();
                                $('.account-social-login').find('.checkout-login').show();
                            }
                            else if(customredirectUrl == 'wishlist'){
                                $('.account-social-login').find('.login-social').hide();
                                $('.account-social-login').find('.wishlist-login').show();
                                wishlistproductId = $(this).attr('data-product');
                                $.cookie('wishlistaddproductlogingoogle', wishlistproductId, {expires: 86400}); 
                            }
                            else {
                                $('.account-social-login').find('.login-social').hide();
                                $('.account-social-login').find('.noramal-login').show();
                            }
                    ...
                    if (self.options.registerForm) {
                        self.options.registerForm.mage(
                            'validation', {
                                messages: {
                                    'password': {'validate-customer-password': 'Min 8 characters with one upper case, one number and one special character.'}
                                },
                                radioCheckboxClosest: '.nested',
                                submitHandler: function (form) {
                                    self.closePopup();
                ...
                redirectAfterLogin: function () {
                    ...
                    var checkoutpageUrl = baseUrl + 'onestepcheckout';
                    var cartUrl = baseUrl + 'checkout/cart';
                    var wishlistUrl = baseUrl + 'wishlist';
                    var addwishlisturl = '';
                    if(wishlistproductId){
                        addwishlisturl = baseUrl + 'customwishlist/index/index/productid/'+wishlistproductId;
                    }
                    
                    if (this.options.ajaxLogin.urlRedirect == 0) {
                        if(customredirectUrl == 'wishlist'){
                            setTimeout(
                                function () {
                                    window.location.replace(addwishlisturl);
                                }, 100
                            );
                        }
                        else if(customredirectUrl == 'checkout'){
                            setTimeout(
                                function () {
